...
import com.example.cleanarchitectureapp.feature_note.domain.use_case.NoteUseCases
        ...
        when(event){
            ...
            is NotesEvent.DeleteNote ->{
                viewModelScope.launch {
                    noteUseCases.deleteNoteCase(event.note)
                    recentlyDeletedNote = event.note
                }
            }
            is NotesEvent.RestoreNote->{
                viewModelScope.launch {
                    noteUseCases.addNoteCase(recentlyDeletedNote ?:return@launch)
